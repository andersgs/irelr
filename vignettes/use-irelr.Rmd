...
ggplot(sims, aes(x = LR99_avg, fill = rel_cat)) +
  ...
  xlab("Lynch and Ritland (1999) index") +
  ylab("Density") +
  scale_fill_discrete(name = "Relatedness categories", labels = c("Full-Sibs", "Half-Sibs", "Unrelated")) +
  theme(legend.justification=c(1,0), legend.position=c(1,0.75))
```
...
```{r, blouin_classifier2}
rel_cats <- c('fs','hs','un')
n_thresholds <- 2
blouin_thresholds <- numeric(n_thresholds)
for(i in 1:n_thresholds) {
  blouin_thresholds[i] <- blouin_classifier(sims_lr99,rel_cats = c('fs', 'hs'))
}
# checking the value
blouin_threshold
